...
using Tyuiu.BabaiKV.Sprint6.Task3.V20.Lib;
namespace Tyuiu.BabaiKV.Sprint6.Task3.V20
{
    public partial class FormMain : Form
    {
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            ...
            int c = mtrx.Length / r;
            dataGridViewMtrx_BKV.ColumnCount = c;
            dataGridViewMtrx_BKV.RowCount = r;
            for (int i = 0; i < c; i++)
            {
                dataGridViewMtrx_BKV.Columns[i].Width = 25;
            }
            for (int i = 0; i < r; i++)
            {
                for (int j = 0; j < c; j++)
                {
                    dataGridViewMtrx_BKV.Rows[i].Cells[j].Value = Convert.ToString(mtrx[i, j]);
                }
            }
        }
        ...
        {
            MessageBox.Show("Таск 3 выполнила студентка группы СМАРТб-24-1 Бабай Ксения Витальевна", "Сообщение", MessageBoxButtons.OK);
        }
